 ...
 *
 * OMP port note: OMP compaction entries carry extension data in `preserveData` (the
 * `session.compacting` hook result), not `details`, so the handoff-id check reads both.
 */

export type HandoffStatus = "pending" | "compacting" | "failed" | "ready" | "done";

export interface Handoff {
	/** Durable id: carried in compaction details/preserveData and in the handoff message details. */
	id: string;
	note: string;
...
/** customType of the TUI-only threshold-crossing record. */
export const PHASE_ENTRY_TYPE = "self-compact-phase";
/** customType of /self-compact-info records. */
...
/** Minimal structural view of the session entries we care about (subset of OMP's SessionEntry). */
...
/** The handoff id a compaction entry carries, wherever the host stored it. */
function compactionHandoffId(entry: EntryLike): string | undefined {
	for (const carrier of [entry.details, entry.preserveData]) {
		if (carrier && typeof carrier === "object" && "handoffId" in carrier && typeof carrier.handoffId === "string") return carrier.handoffId;
	}
	return undefined;
}
...
export function recoverState(entries: EntryLike[]): RecoveredState {
	...
	const compactionLanded = entries.some((e) => e.type === "compaction" && compactionHandoffId(e) === h.id);
