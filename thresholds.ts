 ...
 *
 * OMP port note: `SpecSource` gains "file" for self-compact.json values.
 */
...
export type SpecSource = "flag" | "file" | "default";
...
export function parseSpecs(specs: ThresholdSpecs): ParsedSpecs {
	return {
		soft: parseTokenSpec(specs.softAt, "compactSoftAt"),
		warn: parseTokenSpec(specs.at, "compactAt"),
		buffer: parseTokenSpec(specs.buffer, "compactBuffer"),
	};
}
...
export function validateSpecs(specs: ThresholdSpecs): ParsedSpecs {
	...
	if (parsed.warn.kind === "percent" && parsed.warn.value > capPct) {
		throw new Error(`Invalid compactAt: "${parsed.warn.raw}" is above the ${capPct}% hard cap.`);
	}
	if (parsed.soft.kind === "percent" && parsed.soft.value > capPct) {
		throw new Error(`Invalid compactSoftAt: "${parsed.soft.raw}" is above the ${capPct}% hard cap.`);
	}
	if (parsed.soft.kind === parsed.warn.kind && parsed.soft.value > parsed.warn.value) {
		throw new Error(
			`Invalid thresholds: compactSoftAt (${parsed.soft.raw}) must not exceed compactAt (${parsed.warn.raw}).`,
		);
	}
	return parsed;
}
...
export function resolveThresholds(
	...
	if (warnTokens > capTokens) {
		if (options.fromDefaults) {
			notes.push(
				`Default compactAt (${parsed.warn.raw}) exceeds the ${HARD_CAP_FRACTION * 100}% cap of this ${contextWindow}-token window; clamped to ${capTokens}.`,
			);
			...
			return {
				ok: false,
				error: `Invalid compactAt: ${parsed.warn.raw} (${warnTokens} tokens) exceeds the ${HARD_CAP_FRACTION * 100}% cap (${capTokens} tokens) of the ${contextWindow}-token window.`,
			};
		}
	}
	if (softTokens > warnTokens) {
		if (options.fromDefaults) {
			notes.push(`Default compactSoftAt (${parsed.soft.raw}) exceeds the warning threshold; clamped to ${warnTokens}.`);
			softTokens = warnTokens;
			clamped = true;
		} else {
			return {
				ok: false,
				error: `Invalid thresholds: compactSoftAt (${parsed.soft.raw} = ${softTokens} tokens) must not exceed compactAt (${parsed.warn.raw} = ${warnTokens} tokens).`,
			};
		}
	}
