 ...
 *
 * Files are read fresh on every use so edits apply live. Lookup order (first hit wins):
 *   1. <cwd>/.omp/self-compact/<name>          (project override)
 *   2. <cwd>/.pi/self-compact/<name>           (upstream-Pi layout, honored for parity)
 *   3. ~/.omp/agent/self-compact/<name>        (user-global override)
 *   4. ~/.pi/agent/self-compact/<name>         (upstream-Pi global, honored for parity)
 *   5. <extension dir>/prompts/<name>          (bundled copies of upstream's files)
 *   6. built-in default
 */
...
export const BUILTIN_PROMPTS: Record<PromptKind, string> = {
	summaryInstructions: "",
	soft: `[self-compact · notice] Heads-up only. Context usage is {{used_tokens}} tokens ({{used_percent}}) of {{context_window}}, past the soft line of {{soft_tokens}}. Nothing is blocked and no action is required; keep working. Do not acknowledge, report, or reply to this notice, and do not call \`view_context\` to check the numbers — you will be told when you reach the warning line at {{warning_tokens}}; the hard cutoff is {{forced_tokens}} ({{forced_percent}}). You decide when to compact: \`self_compact\` takes a \`note_to_self\` (up to {{note_max_chars}} chars) with anything you want to see after the compaction, returned to you as is.`,
...
/** --compact-prompt literal > compactPrompt in self-compact.json > file > built-in. */
export function resolveCompactionPrompt(options: { flag?: string; file?: string; searchDirs: string[] }): LoadedPrompt {
	...
	if (file) return { text: file, source: "self-compact.json" };
